...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        last_true_exp = scheme_eval(expressions.first, env)
        if is_scheme_false(last_true_exp):
            return False
        expressions = expressions.rest
    return last_true_exp
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            return scheme_eval(expressions.first, env, True)
        curr_expr = scheme_eval(expressions.first, env)
        if is_scheme_true(curr_expr):
            return curr_expr
        expressions = expressions.rest
    return False
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    names = values = nil
    # BEGIN PROBLEM 14
    #Note: making pair list becuase nil is given
    ...
    #check the formals to make sure there isn't repeat causing error
    # END PROBLEM 14
