...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0, score1) < goal:
        if next_player(who) == 1: #If P1 is next, then the current score we alter is P0
            score0 = score0 + take_turn(strategy0(score0, score1), score1, dice, goal=GOAL_SCORE)
            if score0 == score1:
        ...
        if next_player(who) == 0: #If P0 is next, then the current score we alter is P1
            score1 = score1 + take_turn(strategy1(score1, score0), score0, dice, goal=GOAL_SCORE)
            if score1 == score0:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def f(p0score, p1score):
        last_high = running_high
        if who == 0:
            gain = p0score - last_score
            if gain > last_high:
                last_high = gain
                print(gain, "point(s)! That's a record gain for Player 0!" )
                return announce_highest(0, p0score, last_high)
            else:
                return announce_highest(0, p0score, last_high)
        if who == 1:
            gain = p1score - last_score
            if gain > last_high:
                last_high = gain
                print(gain, "point(s)! That's a record gain for Player 1!" )
                return announce_highest(1, p1score, last_high)
            else:
                return announce_highest(1, p1score, last_high)
    return f
    # END PROBLEM 7
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if picky_piggy(opponent_score) >= cutoff or picky_piggy(opponent_score) + score == opponent_score:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
