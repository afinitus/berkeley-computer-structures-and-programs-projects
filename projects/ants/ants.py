...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        tiles_moved = 0
        while not current_tile.is_hive:
            if self.min_range <= tiles_moved <= self.max_range and current_tile.bees:
                return random_bee(current_tile.bees)
            else:
                tiles_moved += 1
                current_tile = current_tile.entrance
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        current_tile = self.place
        if self.chew_countdown == 0 and current_tile.bees:
            bee = random_bee(current_tile.bees)
            bee.reduce_health(bee.health)
            self.chew_countdown = self.chew_duration
        elif self.chew_countdown > 0 and current_tile.bees:
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.is_queen:
            return None
        else:
            gamestate.is_queen = True
    ...
    def action(self, gamestate):
        ...
        current_tile = self.place.exit
        prior_tile = current_tile.exit
        while current_tile:
            ant_behind = current_tile.ant
            if ant_behind:
                ant_behind.buff()
                if ant_behind.is_container and ant_behind.ant_contained:
                   ant_behind.ant_contained.buff()
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.slow_time and self.scare_time:
            self.slow_time -= 1
            if gamestate.time % 2 == 0:
                self.scare_time -= 1
                bees_tile = self.place
                next_tile = bees_tile.entrance
                if not next_tile.is_hive and not bees_tile.ant:
