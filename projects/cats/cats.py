...
def accuracy(typed, reference):
    ...
    same_words = 0
    lowest_words = typed_words
    if len(typed_words) < len(reference_words):
        lowest_words = typed_words
    elif len(typed_words) > len(reference_words):
        lowest_words = reference_words
    for i in range(len(lowest_words)):
    ...
    if typed == '' and reference == '':
        ...
        return 0.0
    return same_words * 100 / len(typed_words)
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for i in range(len(valid_words)):
        if typed_word == valid_words[i]:
            return typed_word
        elif diff_function(typed_word, valid_words[i], limit) < lim:
            lim  = diff_function(typed_word, valid_words[i], limit)
            closest_word = valid_words[i]
        elif diff_function(typed_word, valid_words[i], limit) <= lim and closest_word == []:
            lim  = diff_function(typed_word, valid_words[i], limit)
            closest_word = valid_words[i]
    if closest_word == []:
        return typed_word
...
def feline_flips(start, goal, limit):
    ...
    5
    """
    def helper(start, goal, differences):
        if limit < differences:
            return limit + 1
        elif goal == '' or start == '':
            differences += abs(len(start) - len(goal))
            return differences
        elif start[0] != goal[0]:
            differences += 1
            return helper(start[1:], goal[1:], differences)
        else:
            return helper(start[1:], goal[1:], differences)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        if sofar[i] == prompt[i]:
            ...
            values = {'id': user_id, 'progress': words_so_far / len(prompt)}
            upload(values)
            return print(values['progress'])
    values = {'id': user_id, 'progress': words_so_far / len(prompt)}
    upload(values)
...
def time_per_word(words, times_per_player):
    ...
    for k in range(len(times_per_player)):
        ...
        for i in range(len(times_per_player[k]) - 1):
            times[k] += [times_per_player[k][i + 1] - times_per_player[k][i]]
...
def fastest_words(match):
    ...
    player_words = []
    for a in player_indices:
        player_words += [[]]
    for k in word_indices:
        min_time = []
        for i in player_indices:
            if min_time == []:
                min_time = time(match, 0, k)
                player = 0
            elif time(match, i, k) < min_time:
                min_time = time(match, i, k)
                player = i
        player_words[player] += [word_at(match, k)]
    return player_words
